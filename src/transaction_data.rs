...
use crate::{
    ...
    sys::{
        BinderBufferFlags, BinderBufferObject, BinderFdArrayObject, BinderFdObject,
...
impl PayloadReader {
    pub fn read_bytes(&mut self, num_bytes: usize) -> Result<&[u8], PayloadBytesReadError> {
        let offsets = unsafe { slice::from_raw_parts(self.offsets_ptr, self.offsets_len) };
...
impl Drop for PayloadReader {
    fn drop(&mut self) {
        // tell the kernel to unload the buffers
        todo!()
    }
}
